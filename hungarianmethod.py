    ...
    for i in range(Rows):
        ...
        if (np.count_nonzero(tickMatrix[i] == 0) == 0):
            for j in range(np.count_nonzero(np.isnan(tickMatrix[i]))):
                ticked_C.append(np.argwhere(np.isnan(tickMatrix[i])).flatten()[j])
            ticked_R.append(i)
            ...
            for j in range(len(pos_nan)):
                ...
                if (np.isnan(tickMatrix[pos_0][0][0]).any()):
                    ...
                    ticked_C.append(np.argwhere(np.isnan(tickMatrix[pos_0[0][0]]))[0][0])
    # print(pos_nan)
    ticked_R = np.unique(ticked_R)
    unticked_R = np.unique(np.delete(unticked_R, np.where(np.in1d(unticked_R, ticked_R))))
...
def assignMatrix(assignMat):
    ...
    if(assignMatrix.counter>30):
        for i in range(Rows):
            if(i<Rows-1):
                if (np.count_nonzero(assignMat[i] == 0) >= 1):
                    pos = np.argwhere(assignMat[i] == 0)
                    for j in range(len(pos)):
                        if (assignMat[i][pos[j][0]] == 0 and assignMat[i+1][pos[j][0]] == 0):
                            continue
                        else:
                            for k in range(Columns):
                                if (assignMat[k, pos[j][0]] == 0):
                                    assignMat[k, pos[j][0]] = np.nan
                                assignMat[i, pos[j][0]] = 0 
            else:
                if (np.count_nonzero(assignMat[i] == 0) >= 1):
                    pos = np.argwhere(assignMat[i] == 0)
                    for j in range(len(pos)-1):
                        if(matClone[i][pos[j][0]]>matClone[i][pos[j+1][0]]):
                            assignMat[i, pos[j][0]]=np.nan
                        else:
                            assignMat[i, pos[j+1][0]]=np.nan
...
print("Matrix After Adding Dummy Row or Column:\n", matrix)
# print("Clone:\n",matClone)

matrix = zeroRow(matrix)
print("Matrix After subtracting minimum of row from each element of that row:\n", matrix)

matrix = zeroColumn(matrix)
print("Matrix After subtracting minimum of column from each element of that column:\n", matrix)
